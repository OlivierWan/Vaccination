...
{
    public sealed class UpdateAuditableEntitiesInterceptor(IHttpContextAccessor httpContextAccessor) : SaveChangesInterceptor
    {
            ...
            foreach (EntityEntry<IAuditableEntity> entityEntry in entries)
            {
                if (entityEntry.State == EntityState.Added)
                {
                    ...
                    if (_httpContextAccessor.HttpContext?.User?.Identity?.IsAuthenticated == true)
                    {
                        ...
                        entityEntry.Property(x => x.CreatedBy).CurrentValue = userId != null ? new Guid(userId) : Guid.Empty;
                    }
                    else
                    {
                ...
                if (entityEntry.State == EntityState.Modified)
                {
                    ...
                    {
                        string? userId = _httpContextAccessor.HttpContext.User.FindFirstValue("userid");
                        entityEntry.Property(x => x.ModifiedBy).CurrentValue = userId != null ? new Guid(userId) : Guid.Empty;
                    }
                    else
                    {
